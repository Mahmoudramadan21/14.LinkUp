...
const HighlightModalPage = () => {
  ...
  // Fetch highlight if not in state
  useEffect(() => {
  ...
  }, [dispatch, username,isAnyModalOpen, highlightId, highlights]);

  // Handle errors with toast + clear
  useEffect(() => {
    const highlightErrors: Array<keyof typeof highlightError> = [
      'getUserHighlightById',
      'updateHighlight',
      'deleteHighlight',
    ];
    const storyErrors: Array<keyof typeof storyError> = ['toggleStoryLike', 'deleteStory'];

    highlightErrors.forEach((key) => {
  ...
  const handleDeleteStory = useCallback(
    (storyId: number) => {
      dispatch(deleteStoryThunk(storyId)).then(() => {
        router.back();
      });
    },
    [dispatch, router]
  ...
  const handleDeleteHighlight = useCallback(
    (highlightId: number) => {
      dispatch(deleteHighlightThunk({ highlightId, username: username as string })).then(() => {
        router.back();
      });
    },
  ...
  const handleRemoveStory = useCallback(
    (storyId: number, highlightId: number) => {
      ...
      const newStoryIds = currentHighlight.stories
  ...
  const handleOpenViewersModal = useCallback((storyId: number) => {
    setShowViewersModal(storyId);
  }, []);
  ...
  return (
    <>
      <HighlightViewerModal
        ...
        onEditHighlight={handleOpenEditModal}
        ...
        <ConfirmationModal
          ...
          actionThunk={(id) => {
            ...
            return updateHighlightThunk({
              ...
              data: {
                storyIds: highlights
                  .find((h) => h.highlightId === numericHighlightId)
                  ?.stories.filter((s) => s.storyId !== id)
                  .map((s) => s.storyId) || [],
              },
              username: username as string,
            });
