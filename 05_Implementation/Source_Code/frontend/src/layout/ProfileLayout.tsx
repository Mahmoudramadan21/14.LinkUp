...
const ProfileLayout: React.FC<ProfileLayoutProps> = ({ children }) => {
  ...
  const isSavedPostsPage = router.pathname.includes('/saved');

  const handleTabChange = useCallback(
    (tab: 'posts' | 'saved') => {
      if (!username) return;
      const path = tab === 'posts' ? `/profile/${username}` : `/profile/${username}/saved`;
        ...
        <header className="profile-page__header">
          ...
          <nav className="profile-page__tabs" aria-label="Profile content tabs">
            <button
              ...
              className={`profile-page__tab-btn ${!isSavedPostsPage ? 'profile-page__tab-btn--active' : ''}`}
              aria-label="View posts"
              aria-current={!isSavedPostsPage ? 'true' : undefined}
            >
              <Image
                src={!isSavedPostsPage ? '/icons/menu-active.svg' : '/icons/menu.svg'}
                alt="Posts tab icon"
                width={24}
                height={24}
            ...
            {isOwnProfile && (
              ...
              >
                <Image
                  src={isSavedPostsPage ? '/icons/saved.svg' : '/icons/save.svg'}
