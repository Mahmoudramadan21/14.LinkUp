...
const CreateStoryModal: React.FC<CreateStoryModalProps> = ({ isOpen, onClose }) => {
  ...
  const drawCanvas = useCallback(
    (state: CanvasState) => {
      ...
      if (!ctx) {
        console.error('Canvas context not available in drawCanvas');
        return;
      }
      ...
      try {
        ...
        const fontPromises = fonts.map((font) =>
          document.fonts
            ...
            .catch((error) => {
              console.warn(`Failed to load font: ${font}, falling back to system font`, error);
        ...
        if (failed.length > 0) {
          ...
          toast.warn('Some fonts could not be loaded, using system defaults');
        } else {
        ...
        console.error('Unexpected error during font loading:', error);
        toast.error('Error loading fonts, using system defaults');
  ...
  useEffect(() => {
    if (isOpen && canvasRef.current && isFontsLoaded) {
      console.log('Redrawing canvas with state:', canvasState);
      drawCanvas(canvasState);
    }
  ...
  const handleMediaUpload = async (file: File | null) => {
    ...
    try {
      ...
      toast.error((error as Error).message || 'Failed to load image');
    } finally {
  ...
  const getTextBounds = useCallback(
    (text: TextObject, ctx: CanvasRenderingContext2D) => {
      if (!ctx) {
        console.error('Canvas context not available in getTextBounds');
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      const ctx = canvasRef.current.getContext('2d');
      if (!ctx) {
        console.error('Canvas context not available in handlePointerDown');
        return;
      }
      ...
      for (let i = canvasState.texts.length - 1; i >= 0; i--) {
        ...
        if (x >= bounds.x && x <= bounds.x + bounds.width && y >= bounds.y && y <= bounds.y + bounds.height) {
          textIndex = i;
          console.log(`Text hit detected: "${text.content}" at index ${i}`);
          break;
        }
      }
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      const ctx = canvasRef.current.getContext('2d');
      if (!ctx) {
        console.error('Canvas context not available in handlePointerMove');
        return;
      }
  ...
  const addText = useCallback(
    (e?: React.PointerEvent<HTMLCanvasElement>) => {
      if (!canvasRef.current) {
        console.error('Cannot add text: canvas missing');
        toast.error('Canvas not ready');
        return;
      }
      if (!textInput.trim()) {
        console.error('Cannot add text: textInput is empty');
        toast.error('Please enter text');
        return;
      }
      if (!isFontsLoaded) {
        console.error('Cannot add text: fonts not loaded');
      ...
      const ctx = canvasRef.current.getContext('2d');
      if (!ctx) {
        console.error('Canvas context not available in addText');
        return;
      }
  ...
  const handleSubmit = useCallback(() => {
    if (!canvasRef.current) {
      console.error('Canvas not ready for submit');
      toast.error('Canvas not ready');
      return;
    }
    if (!user) {
    ...
    const dataUrl = canvasRef.current.toDataURL('image/webp', 0.92); // Use webp for better compression/performance
    fetch(dataUrl)
      .then((res) => res.blob())
      .then((blob) => {
        const file = new File([blob], 'story.webp', { type: 'image/webp' });
        const request: CreateStoryRequest = { media: file };
        dispatch(createStoryThunk(request))
          .unwrap()
          .then(() => {
        ...
        <div className={styles['create-story-modal__content']}>
          {mode === null ? (
            <div className={styles['create-story-modal__mode-selector']}>
              ...
              >
                <FaPlus size={28} className={styles['create-story-modal__mode-icon']} />
                Add Image
                <input
                  id="media-upload"
                  type="file"
